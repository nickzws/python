...
3. list元素相对于PHP中的数据，从0开始，取出最后一个classmate\[-1\],len\(classmates\) - 1

4. 从最后添加元素 \` classmate.append\('4'\)\`

5. 插入知道位置 \`classmate.insert\(1,'care'\)\`

6. 删除末尾的元素 pop\(\) \`classmate.pop\(\)\`，返回的是插入 弹出后的元素

7. 删除指定位置的元素 pop\(i\) \` classmate.pop\(1\)\`，返回的是弹出的元素

8. 替换：就是重新赋值 \` classmate\[1\] = 0\`

9. 元素里可以是字符串、整数、布尔值，也可以是另一个list（二维数组）、空元素的长度是0
...
3. tuple元素不变具体指的是指向永远不变，比如tuple有个元素是list，list的内容可变。
